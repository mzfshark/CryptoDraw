...
describe("PriceOracle - Coverage", function () {
  ...
  it("should add and remove tokens and convert prices", async function () {
    ...
    const usd = await priceOracle.convertToUSD(
      token.address,
      ethers.utils.parseUnits("1", 18),
    );
    expect(usd).to.equal(ethers.utils.parseEther("2")); // 1 token * $2

    const tokens = await priceOracle.convertFromUSD(
      token.address,
...
describe("PriceOracle - Decimals and Band feed (merged)", function () {
  ...
  it("converts correctly for token with 6 decimals", async function () {
    ...
    const tokenAmount = await priceOracle.convertFromUSD(
      token6.address,
...
describe("PriceOracle - Extended Coverage (merged)", function () {
  ...
  it("onlyOwner enforcement for admin functions", async function () {
    await expect(
      priceOracle
        .connect(user1)
        .addToken(mockToken.address, 18, ethers.utils.parseEther("1")),
  ...
  it("convertFromUSD handles decimals > 18 (e.g., 30) correctly", async function () {
    ...
    const amount = await priceOracle.convertFromUSD(
      token30.address,
      ethers.utils.parseEther("1"),
    );
  ...
  it("price update edge cases", async function () {
    ...
    await expect(
      priceOracle
        .connect(owner)
        .updatePrice(nonExistent.address, ethers.utils.parseEther("1")),
  ...
  it("batch updatePrices: length mismatch, unsupported token, invalid price, success", async function () {
    ...
    await expect(
      priceOracle.updatePrices([t1.address], [ethers.utils.parseEther("1")]),
    ).to.be.revertedWithCustomError(priceOracle, "TokenNotSupported");

    // add support and then invalid price in batch
    await priceOracle.addToken(t1.address, 18, ethers.utils.parseEther("1"));
    await expect(
      priceOracle.updatePrices([t1.address], [ethers.constants.Zero]),
    ).to.be.revertedWithCustomError(priceOracle, "InvalidPrice");
    ...
    await expect(
      priceOracle.updatePrices(
        [ethers.constants.AddressZero, t1.address],
        [newNative, newT1],
      ),
    ).to.not.be.reverted;
  ...
  it("setBandFeed error branches: ZeroAddress and invalid pair", async function () {
    ...
    await expect(
      priceOracle.setBandFeed(mockToken.address, mockToken.address, "", ""),
  ...
  it("cannot remove native ONE and removing unsupported token reverts", async function () {
    ...
    await expect(
      priceOracle.removeToken(ethers.constants.AddressZero),
    ).to.be.revertedWith("Cannot remove native ONE");
    ...
    await expect(
      priceOracle.removeToken(t2.address),
    ).to.be.revertedWithCustomError(priceOracle, "TokenNotSupported");
  });
  ...
  it("rejects zero max age", async function () {
    await expect(priceOracle.setMaxPriceAge(0)).to.be.revertedWith(
      "Invalid max age",
    );
  });
  ...
  it("very small and large amount conversions", async function () {
    ...
    const tokenFromSmallUSD = await priceOracle.convertFromUSD(
      mockToken.address,
      small,
    );
    ...
    await expect(priceOracle.convertToUSD(mockToken.address, largeAmount)).to
      .not.be.reverted;
    const largeUSD = ethers.utils.parseEther("1000000");
    await expect(priceOracle.convertFromUSD(mockToken.address, largeUSD)).to.not
      .be.reverted;
  });

  it("native token handling and updates", async function () {
    const nativePrice = await priceOracle.getUSDPrice(
      ethers.constants.AddressZero,
    );
  ...
  it("Band Oracle integration: set and clear feeds and for native", async function () {
    ...
    const nativeCfg = await priceOracle.feedConfig(
      ethers.constants.AddressZero,
    );
  ...
  it("emits events on updates and configuration", async function () {
    ...
    await expect(
      priceOracle.updatePrice(mockToken.address, ethers.utils.parseEther("2")),
  ...
  it("convertToUSD/convertFromUSD use Band feed price and revert to MANUAL after clearFeed", async function () {
    ...
    let usd = await priceOracle.convertToUSD(
      mockToken.address,
      ethers.utils.parseEther("1"),
    );
    ...
    usd = await priceOracle.convertToUSD(
      mockToken.address,
      ethers.utils.parseEther("1"),
    );
    expect(usd).to.equal(ethers.utils.parseEther("2"));
    const tokensFrom2USD = await priceOracle.convertFromUSD(
      mockToken.address,
      ethers.utils.parseEther("2"),
    );
  ...
  it("BAND refTime picks the minimum of base/quote timestamps (true branch)", async function () {
    ...
    const v = await priceOracle.convertToUSD(
      mockToken.address,
      ethers.utils.parseEther("1"),
    );
  ...
  it("BAND pricing path with token decimals < 18 (6 decimals)", async function () {
    ...
    const back = await priceOracle.convertFromUSD(
      token6.address,
      ethers.utils.parseEther("2"),
    );
