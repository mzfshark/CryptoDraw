...
describe("CryptoDrawV2 Contract", function () {
  ...
  async function deployCryptoDrawV2Fixture() {
    ...
    const CryptoDrawV2 = await ethers.getContractFactory(
  ...
  describe("Deployment", function () {
    it("Should set the right owner", async function () {
      const { cryptoDrawV2, owner } = await loadFixture(
        deployCryptoDrawV2Fixture,
      );
      expect(
        await cryptoDrawV2.hasRole(
          await cryptoDrawV2.DEFAULT_ADMIN_ROLE(),
          owner.address,
        ),
      ).to.be.true;
    });
    ...
    it("Should initialize with correct addresses", async function () {
      const { cryptoDrawV2, ticketNFT, priceOracle } = await loadFixture(
        deployCryptoDrawV2Fixture,
      );
  ...
  describe("Game Configuration", function () {
    it("Should allow admin to configure SuperSeven game", async function () {
      ...
      await cryptoDrawV2
        .connect(owner)
        .setGameConfig(gameType, ticketPriceUSD, drawInterval, true);
    ...
    it("Should prevent non-admin from configuring games", async function () {
      const { cryptoDrawV2, user1 } = await loadFixture(
        deployCryptoDrawV2Fixture,
      );
  ...
  describe("Payment Token Management", function () {
    it("Should allow admin to add supported payment token", async function () {
      ...
      await cryptoDrawV2
        .connect(owner)
        .setSupportedToken(mockToken.address, true);
    ...
    it("Should allow admin to remove supported payment token", async function () {
      ...
      await cryptoDrawV2
        .connect(owner)
        .setSupportedToken(mockToken.address, false);
  ...
  describe("Draw Management", function () {
    ...
    it("Should allow operator to create new draw", async function () {
      const { cryptoDrawV2, operator } = await loadFixture(
        deployCryptoDrawV2Fixture,
      );
      await cryptoDrawV2.setGameConfig(
        0,
        ethers.utils.parseEther("1"),
        24 * 60 * 60,
        true,
    ...
    it("Should prevent non-operator from creating draws", async function () {
      const { cryptoDrawV2, owner, user1 } = await loadFixture(
        deployCryptoDrawV2Fixture,
      );
      await cryptoDrawV2
        .connect(owner)
        .setGameConfig(0, ethers.utils.parseEther("1"), 24 * 60 * 60, true);

      await expect(cryptoDrawV2.connect(user1).createDraw(0)).to.be.reverted;
    });

    it("Should allow operator to close draw", async function () {
      const { cryptoDrawV2, owner, operator } = await loadFixture(
        deployCryptoDrawV2Fixture,
      );
      await cryptoDrawV2
        .connect(owner)
    ...
    it("Should emit DrawCreated event", async function () {
      ...
      await expect(cryptoDrawV2.connect(operator).createDraw(0)).to.emit(
        cryptoDrawV2,
        "DrawCreated",
      );
    });
    ...
    it("Should emit DrawClosed event", async function () {
      ...
      await expect(cryptoDrawV2.connect(operator).closeDrawSimple(0, 1))
  ...
  describe("Ticket Purchase - Native ETH", function () {
    ...
    it("Should allow buying SuperSeven ticket with ETH", async function () {
      const { cryptoDrawV2, owner, operator, user1, ticketNFT, priceOracle } =
        await loadFixture(deployCryptoDrawV2Fixture);
    ...
    it("Should reject invalid SuperSeven numbers (wrong count)", async function () {
      const { cryptoDrawV2, owner, operator, user1, priceOracle } =
        await loadFixture(deployCryptoDrawV2Fixture);
  ...
  describe("Ticket Purchase - ERC20 Token", function () {
    ...
    it("Should allow buying EasyLotto ticket with ERC20 token", async function () {
      const { cryptoDrawV2, user1, ticketNFT, owner, priceOracle } =
        await loadFixture(deployCryptoDrawV2Fixture);
      ...
      await priceOracle
        .connect(owner)
        .addToken(mockToken.address, 6, ethers.utils.parseEther("1"));
  ...
  describe("Agent System", function () {
    it("Should calculate agent commission correctly", async function () {
      const { cryptoDrawV2, owner, agent, user1, priceOracle } =
        await loadFixture(deployCryptoDrawV2Fixture);
      ...
      const expectedCommission = ethers.utils
        ...
        .mul(861)
        .div(10000); // 8.61% of $1
      expect(commission).to.be.closeTo(
        expectedCommission,
    ...
    it("Should allow agent to withdraw commission", async function () {
      ...
      const commissionBefore = await cryptoDrawV2.agentCommissions(
        agent.address,
      );
      ...
      await user1.sendTransaction({
        ...
        value: commissionBefore,
      });

      await cryptoDrawV2.connect(agent).withdrawAgentCommission();

      const commissionAfter = await cryptoDrawV2.agentCommissions(
        agent.address,
      );
  ...
  describe("Revenue Distribution", function () {
    it("Should distribute revenue according to configuration", async function () {
      ...
      await cryptoDrawV2
        .connect(owner)
        .setGameConfig(0, ethers.utils.parseEther("10"), 24 * 60 * 60, true); // $10 ticket
      await cryptoDrawV2.connect(operator).createDraw(0);
      await priceOracle.updatePrice(
        ethers.constants.AddressZero,
        ethers.utils.parseEther("2000"),
    ...
    it("Should allow admin to update revenue configuration", async function () {
      ...
      expect(updatedConfig.agentCommissionPercent).to.equal(
        newConfig.agentCommissionPercent,
      );
    });
  ...
  describe("Merged v2 - Priority flows", function () {
    ...
    it("buyTicketWithToken wrapper works using ERC20", async function () {
      ...
      await mockToken
        .connect(user1)
        .approve(cryptoDraw.address, ethers.utils.parseEther("1"));
    ...
    it("claimPrize reverts when caller is not ticket owner", async function () {
      const { user1, user2, cryptoDraw, mockToken } =
        await deployMergedFixture();
      await mockToken
        .connect(user1)
        .approve(cryptoDraw.address, ethers.utils.parseEther("1"));
      const tx = await cryptoDraw
    ...
    it("buyTicket native: underpay, exact and overpay refund", async function () {
      ...
      expect(after).to.be.closeTo(
        before.sub(ethers.utils.parseEther("1")).sub(gas),
        ethers.utils.parseEther("0.01"),
      );
    });
    ...
    it("closeDrawSimple emits DrawClosed and closeDraw emits DrawCompleted + RevenueDistributed", async function () {
      ...
      const rcpt = await tx.wait();
      expect(rcpt.events.find((e) => e.event === "DrawClosed")).to.not.be
        .undefined;
      ...
      const tx2 = await cryptoDraw
        .connect(operator)
        ["closeDraw(uint8,uint32,uint256)"](0, drawId2, 1);
      const rcpt2 = await tx2.wait();
      expect(rcpt2.events.find((e) => e.event === "DrawCompleted")).to.not.be
        .undefined;
      expect(rcpt2.events.find((e) => e.event === "RevenueDistributed")).to.not
        .be.undefined;
    });
  ...
  describe("Coverage add-ons", function () {
    ...
    function computeSuperSevenWinning(randomnessBn, drawIdBn) {
      let seed = ethers.utils.solidityKeccak256(
        ["uint256", "uint32"],
        [randomnessBn, drawIdBn],
      );
      const digits = [];
      for (let col = 0; col < 7; col++) {
        seed = ethers.utils.solidityKeccak256(
          ["bytes32", "uint8"],
          [seed, col],
        );
    ...
    async function baseFixture() {
      ...
      await ticketNFT.setCryptoDrawAddress(cryptoDraw.address);
      await cryptoDraw.grantRole(
        await cryptoDraw.OPERATOR_ROLE(),
        operator.address,
      );
    ...
    it("AgentSuspended emits event and blocks earning (custom error)", async function () {
      ...
      await expect(cryptoDraw.setSuspendedAgent(agent.address, true))
        .to.emit(cryptoDraw, "AgentStatusUpdated")
    ...
    it("withdrawPrize reverts when no balance and succeeds after claim", async function () {
      const { cryptoDraw, owner, operator, user1, priceOracle } =
        await baseFixture();
      await expect(
      ...
      const ticketId = rcptBuy.events.find((e) => e.event === "TicketPurchased")
        .args.ticketId;
      // close and set winners
      await cryptoDraw
        .connect(operator)
        ["closeDraw(uint8,uint32,uint256)"](0, drawId, randomness);
      // claim
    ...
    it("claimPrize before completion reverts and second call after redeem reverts with TicketAlreadyRedeemed", async function () {
      ...
      // not completed
      await expect(
        cryptoDraw.connect(user1).claimPrize(tokenId),
      ...
      const t2 = r2.events.find((e) => e.event === "TicketPurchased").args
        .ticketId;
      await cryptoDraw
        .connect(operator)
        ["closeDraw(uint8,uint32,uint256)"](0, drawId, randomness);
      ...
      await expect(
        cryptoDraw.connect(user1).claimPrize(t2),
      ).to.be.revertedWithCustomError(cryptoDraw, "TicketAlreadyRedeemed");
    });

    it("withdrawAgentCommission reverts when none available", async function () {
      const { cryptoDraw, agent } = await baseFixture();
      await expect(
        cryptoDraw.connect(agent).withdrawAgentCommission(),
      ).to.be.revertedWithCustomError(cryptoDraw, "NoWithdrawableBalance");
    });
    ...
    it("emergencyWithdraw with ERC20 transfers and emits event", async function () {
      ...
      await expect(
        cryptoDraw.emergencyWithdraw(
          token.address,
          owner.address,
          ethers.utils.parseEther("3"),
        ),
      )
        .to.emit(cryptoDraw, "EmergencyWithdrawal")
        .withArgs(token.address, owner.address, ethers.utils.parseEther("3"));
    ...
    it("EasyLotto prize calculation branch (15 matches) and withdraw", async function () {
      ...
      const rcpt = await tx.wait();
      const tId = rcpt.events.find((e) => e.event === "TicketPurchased").args
        .ticketId;
      // close and claim
      await cryptoDraw
        .connect(operator)
        ["closeDraw(uint8,uint32,uint256)"](1, drawId, randomness);
    ...
    it("claimPrize returns 0 prize (no event) when no matches thresholds met", async function () {
      ...
      const rcpt = await tx.wait();
      const tokenId = rcpt.events.find((e) => e.event === "TicketPurchased")
        .args.ticketId;
    ...
    it("EasyLotto prize branches for 14/13/12/11 matches", async function () {
      ...
      async function runCase(picked) {
        ...
        const tid = rc.events.find((e) => e.event === "TicketPurchased").args
          .ticketId;
        await cryptoDraw
          .connect(operator)
          ["closeDraw(uint8,uint32,uint256)"](1, drawId, randomness);
      ...
      function withDifferences(n) {
        // Build complement numbers from 1..25 that are not in winning
        const set = new Set(winning);
        const complement = [];
        for (let i = 1; i <= 25; i++) {
          if (!set.has(i)) complement.push(i);
        }
        // Replace first n positions with first n complement numbers
        const picked = [...winning];
        for (let i = 0; i < n; i++) {
          picked[i] = complement[i];
        }
        // Ensure still 15 unique in range 1..25
        const uniq = new Set(picked);
        if (uniq.size !== 15)
          throw new Error("constructed invalid EasyLotto pick");
        return picked;
      }
      ...
      const p13 = await (async () => {
        const required = ethers.utils.parseEther("0.001");
        // Build 13 matches: replace 2 numbers with complement
        const set = new Set(win2);
        const complement = [];
        for (let i = 1; i <= 25; i++) if (!set.has(i)) complement.push(i);
        const picked = [...win2];
        picked[0] = complement[0];
        picked[1] = complement[1];
        const tx = await cryptoDraw
          .connect(user1)
          .buyTicket(
            1,
        ...
        await cryptoDraw
          .connect(operator)
          ["closeDraw(uint8,uint32,uint256)"](1, drawId2, randomness2);
      ...
      const p12 = await (async () => {
        const required = ethers.utils.parseEther("0.001");
        const set = new Set(win3);
        const comp = [];
        for (let i = 1; i <= 25; i++) if (!set.has(i)) comp.push(i);
        const arr = [...win3];
        for (let i = 0; i < 3; i++) arr[i] = comp[i];
        const tx = await cryptoDraw
          .connect(user1)
          .buyTicket(
            1,
        ...
        await cryptoDraw
          .connect(operator)
          ["closeDraw(uint8,uint32,uint256)"](1, drawId3, randomness3);
        await (await cryptoDraw.connect(user1).claimPrize(tid)).wait();
        return await cryptoDraw.withdrawableBalances(user1.address);
      })();
      ...
      const p11 = await (async () => {
        const required = ethers.utils.parseEther("0.001");
        const set = new Set(win4);
        const comp = [];
        for (let i = 1; i <= 25; i++) if (!set.has(i)) comp.push(i);
        const arr = [...win4];
        for (let i = 0; i < 4; i++) arr[i] = comp[i];
        const tx = await cryptoDraw
          .connect(user1)
          .buyTicket(
            1,
        ...
        await cryptoDraw
          .connect(operator)
          ["closeDraw(uint8,uint32,uint256)"](1, drawId4, randomness4);
        await (await cryptoDraw.connect(user1).claimPrize(tid)).wait();
        return await cryptoDraw.withdrawableBalances(user1.address);
      })();
    ...
    it("configureGame wrapper (admin-only) updates config", async function () {
      ...
      await expect(
        cryptoDraw
          .connect(user1)
          .configureGame(0, ethers.utils.parseEther("1"), 999, true),
      ).to.be.reverted;
      // owner can call
      await cryptoDraw.configureGame(
        0,
        ethers.utils.parseEther("1"),
        999,
        true,
    ...
    it("setWallets updates all non-zero wallets (treasury, prize, project, grant, operation)", async function () {
      const { cryptoDraw, owner, operator, agent, user1, user2 } =
        await baseFixture();
      // use distinct addresses for each wallet from available signers
      const [treasury, prize, project, grant, operation] = [
        user1.address,
        user2.address,
        operator.address,
        agent.address,
      ...
      await cryptoDraw
        .connect(owner)
        .setWallets(treasury, prize, project, grant, operation);
    ...
    it("SuperSeven prize branches for 6 and 5 matches", async function () {
      const { cryptoDraw, operator, owner, user1 } = await baseFixture();
      // Configure SuperSeven
      await cryptoDraw.setGameConfig(
        0,
        ethers.utils.parseEther("1"),
        86400,
      ...
      // Case: 6 matches (replace 1 digit)
      await cryptoDraw.connect(operator).createDraw(0);
      const drawId1 = await cryptoDraw.getCurrentDrawId(0);
      const rand1 = ethers.BigNumber.from("111111");
      const winDigits1 = computeSuperSevenWinning(rand1, drawId1);
      const pick6 = [...winDigits1];
      pick6[0] = (winDigits1[0] + 1) % 10; // differ one column
      const pay = ethers.utils.parseEther("0.0005");
      let tx = await cryptoDraw
        .connect(user1)
        .buyTicket(
          0,
          pick6,
      ...
      let tid = rc.events.find((e) => e.event === "TicketPurchased").args
        .ticketId;
      await cryptoDraw
        .connect(operator)
        ["closeDraw(uint8,uint32,uint256)"](0, drawId1, rand1);
      await (await cryptoDraw.connect(user1).claimPrize(tid)).wait();
      const owed6 = await cryptoDraw.withdrawableBalances(user1.address);
      expect(owed6).to.be.gt(0);
      await owner.sendTransaction({ to: cryptoDraw.address, value: owed6 });
      await cryptoDraw.connect(user1).withdrawPrize();

      // Case: 5 matches (replace 2 digits)
      await cryptoDraw.connect(operator).createDraw(0);
      const drawId2 = await cryptoDraw.getCurrentDrawId(0);
      const rand2 = ethers.BigNumber.from("222222");
      const winDigits2 = computeSuperSevenWinning(rand2, drawId2);
      const pick5 = [...winDigits2];
      pick5[0] = (winDigits2[0] + 1) % 10;
      pick5[1] = (winDigits2[1] + 2) % 10;
      tx = await cryptoDraw
        .connect(user1)
        .buyTicket(
          0,
      ...
      await cryptoDraw
        .connect(operator)
        ["closeDraw(uint8,uint32,uint256)"](0, drawId2, rand2);
      await (await cryptoDraw.connect(user1).claimPrize(tid)).wait();
