...
describe("TicketNFT - Coverage", function () {
  ...
  it("should mint and provide tokenURI and enforce soulbound restrictions", async function () {
    ...
    const TestTicketNFCCaller = await ethers.getContractFactory(
      "TestTicketNFCCaller",
    );
...
describe("TicketNFT Contract (aligned with current API)", function () {
  async function deployTicketNFTFixture() {
    const [owner, cryptoDraw, user1, user2, attacker] =
  ...
  describe("Access control: onlyCryptoDraw", function () {
    it("owner can set CryptoDraw address", async function () {
      const { ticketNFT, owner, cryptoDraw } = await loadFixture(
        deployTicketNFTFixture,
      );
    ...
    it("non-owner cannot set CryptoDraw address", async function () {
      const { ticketNFT, user1, cryptoDraw } = await loadFixture(
        deployTicketNFTFixture,
      );
      await expect(
    ...
    it("reverts when setting CryptoDraw address to zero", async function () {
      ...
      await expect(
        ticketNFT
          .connect(owner)
          .setCryptoDrawAddress(ethers.constants.AddressZero),
  ...
  describe("Minting", function () {
    it("only CryptoDraw can mint", async function () {
      const { ticketNFT, owner, cryptoDraw, user1 } = await loadFixture(
        deployTicketNFTFixture,
      );
      await ticketNFT.connect(owner).setCryptoDrawAddress(cryptoDraw.address);

      await expect(ticketNFT.connect(user1).mint(user1.address, 1, 12345, 1, 1))
  ...
  describe("Soulbound behavior (no transfers)", function () {
    it("transfers revert", async function () {
      const { ticketNFT, owner, cryptoDraw, user1, user2 } = await loadFixture(
        deployTicketNFTFixture,
      );
      await ticketNFT.connect(owner).setCryptoDrawAddress(cryptoDraw.address);
      await ticketNFT.connect(cryptoDraw).mint(user1.address, 1, 12345, 1, 1);
  ...
  describe("Non-existent token queries", function () {
    ...
    it("getTicketStatus reverts with TokenNotExists for non-existent token", async function () {
      const { ticketNFT } = await loadFixture(deployTicketNFTFixture);
      await expect(
        ticketNFT.getTicketStatus(999999),
...
describe("TicketNFT - Extended Coverage (merged)", function () {
  ...
  it("reverts when minting to zero address", async function () {
    await expect(
      ticketNFT
        .connect(minter)
        .mint(ethers.constants.AddressZero, 0, 12345, 1, 1),
    ).to.be.revertedWith("Invalid recipient");
  });

  it("accepts rounds at boundaries and rejects invalid ones", async function () {
    await expect(ticketNFT.connect(minter).mint(user1.address, 0, 12345, 1, 1))
      .to.not.be.reverted;
    await expect(ticketNFT.connect(minter).mint(user1.address, 0, 12345, 1, 6))
      .to.not.be.reverted;
    await expect(
  ...
  it("getTicket returns full info and reverts for non-existent", async function () {
    const r = await (
      await ticketNFT.connect(minter).mint(user1.address, 1, 11111, 10, 5)
    ).wait();
    ...
    await expect(ticketNFT.getTicket(999999)).to.be.revertedWithCustomError(
      ticketNFT,
      "TokenNotExists",
    );
  });
  ...
  it("updateStatus emits event and handles all statuses", async function () {
    ...
    const id = r.events.find((e) => e.event === "TicketMinted").args.tokenId;
    await expect(ticketNFT.connect(minter).updateStatus(id, 2)).to.emit(
      ticketNFT,
      "TicketStatusUpdated",
    );
    for (let s = 0; s <= 3; s++) {
  ...
  it("burn emits event and reverts on invalid token", async function () {
    ...
    const id = r.events.find((e) => e.event === "TicketMinted").args.tokenId;
    await expect(ticketNFT.connect(minter).burn(id)).to.emit(
      ticketNFT,
      "TicketBurned",
    );
    await expect(ticketNFT.ownerOf(id)).to.be.revertedWith(
    ...
    await expect(
      ticketNFT.connect(minter).burn(id),
    ).to.be.revertedWithCustomError(ticketNFT, "TokenNotExists");
  });

  it("decrementRounds down to zero then reverts beyond", async function () {
    const r = await (
      await ticketNFT.connect(minter).mint(user1.address, 0, 12345, 1, 2)
    ).wait();
    const id = r.events.find((e) => e.event === "TicketMinted").args.tokenId;
    ...
    await expect(
      ticketNFT.connect(minter).decrementRounds(id),
  ...
  it("tokenURI encodes JSON and reflects status changes", async function () {
    ...
    const jsonRedeemed = Buffer.from(
      uriRedeemed.split(",")[1],
      "base64",
    ).toString("utf8");
  ...
  it("getTicketBasic returns tuple and tokenURI reflects 'Burned' when status set via updateStatus", async function () {
    const r = await (
      await ticketNFT.connect(minter).mint(user1.address, 0, 12345, 42, 1)
    ).wait();
    const id = r.events.find((e) => e.event === "TicketMinted").args.tokenId;
  ...
  it("isTicketActive returns true only when ACTIVE and roundsRemaining > 0", async function () {
    await ticketNFT.setCryptoDrawAddress(minter.address);
    const r = await (
      await ticketNFT.connect(minter).mint(user1.address, 0, 111, 1, 2)
    ).wait();
    const id = r.events.find((e) => e.event === "TicketMinted").args.tokenId;
  ...
  it("transferFrom reverts with custom error TransferNotAllowed", async function () {
    ...
    const id = r.events.find((e) => e.event === "TicketMinted").args.tokenId;
    await expect(
      ticketNFT.connect(user1).transferFrom(user1.address, user2.address, id),
    ).to.be.revertedWithCustomError(ticketNFT, "TransferNotAllowed");
    await ticketNFT.connect(user1).approve(user2.address, id);
    await expect(
      ticketNFT.connect(user2).transferFrom(user1.address, user2.address, id),
    ).to.be.revertedWithCustomError(ticketNFT, "TransferNotAllowed");
  });
