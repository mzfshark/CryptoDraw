...
describe("AgentProxy Contract", function () {
  ...
  async function deployAgentProxyFixture() {
    const [owner, agent1, agent2, user1, user2, attacker] =
  ...
  describe("Deployment", function () {
    ...
    it("Should set the correct CryptoDraw address", async function () {
      const { agentProxy, cryptoDrawMock } = await loadFixture(
        deployAgentProxyFixture,
      );
  ...
  describe("Agent Registration", function () {
    it("Should allow owner to register agent", async function () {
      const { agentProxy, owner, agent1 } = await loadFixture(
        deployAgentProxyFixture,
      );
      ...
      await agentProxy
        .connect(owner)
        .registerAgent(agent1.address, commissionRate);
    ...
    it("Should prevent non-owner from registering agents", async function () {
      const { agentProxy, agent1, agent2 } = await loadFixture(
        deployAgentProxyFixture,
      );
    ...
    it("Should prevent registering zero address as agent", async function () {
      ...
      await expect(
        agentProxy
          .connect(owner)
          .registerAgent(ethers.constants.AddressZero, 500),
  ...
  describe("Agent Deactivation", function () {
    ...
    it("Should emit AgentDeactivated event", async function () {
      ...
      await expect(agentProxy.connect(owner).deactivateAgent(agent1.address))
    ...
    it("Should prevent non-owner from deactivating agents", async function () {
      const { agentProxy, owner, agent1, agent2 } = await loadFixture(
        deployAgentProxyFixture,
      );
      await agentProxy.connect(owner).registerAgent(agent1.address, 500);
    ...
    it("Should allow deactivating non-existent agent (no-op)", async function () {
      ...
      await expect(agentProxy.connect(owner).deactivateAgent(agent1.address)).to
        .not.be.reverted;
    });
  });
  ...
  describe("Ticket Purchase Through Agent", function () {
    ...
    it("Should allow buying ticket through registered agent", async function () {
      const { agentProxy, owner, agent1, user1, cryptoDrawMock } =
        await loadFixture(deployAgentProxyFixture);
      await agentProxy.connect(owner).registerAgent(agent1.address, 500);
      ...
      const tx = await agentProxy
        .connect(user1)
        .buyTicketThroughAgent(gameType, numbers, rounds, agent1.address, {
          value: ticketPrice,
        });

      const receipt = await tx.wait();
      const event = receipt.events.find(
        (e) => e.event === "TicketPurchasedThroughAgent",
      );
    ...
    it("Should revert with TransferFailed if receiver rejects ETH", async function () {
      const { agentProxy, owner, user1, cryptoDrawMock } = await loadFixture(
        deployAgentProxyFixture,
      );
      ...
      await agentProxy
        .connect(owner)
        .registerAgent(revertingAgent.address, 1000);
      const ticketPrice = ethers.utils.parseEther("1");
      await cryptoDrawMock.setTicketPrice(ticketPrice);

      await agentProxy
        .connect(user1)
        .buyTicketThroughAgent(
          1,
    ...
    it("Should calculate and distribute commission correctly", async function () {
      ...
      const agentBalanceBefore = await ethers.provider.getBalance(
        agent1.address,
      );
    ...
    it("Should prevent buying through inactive agent", async function () {
      const { agentProxy, owner, agent1, user1 } = await loadFixture(
        deployAgentProxyFixture,
      );
    ...
    it("Should handle multiple purchases through same agent", async function () {
      const { agentProxy, owner, agent1, user1, user2, cryptoDrawMock } =
        await loadFixture(deployAgentProxyFixture);
  ...
  describe("Commission Management", function () {
    ...
    it("Should emit DefaultCommissionRateUpdated event", async function () {
      ...
      await expect(agentProxy.connect(owner).setDefaultCommissionRate(newRate))
    ...
    it("Should allow updating individual agent commission rate", async function () {
      ...
      await agentProxy
        .connect(owner)
        .updateAgentCommissionRate(agent1.address, 750);
    ...
    it("Should revert when setting agent commission rate above max", async function () {
      ...
      await expect(
        agentProxy
          .connect(owner)
          .updateAgentCommissionRate(agent1.address, 2001),
      ).to.be.revertedWith("InvalidCommissionRate");
    });
  });
  ...
  describe("Commission Withdrawal", function () {
    ...
    it("Should allow agents to withdraw their commissions", async function () {
      ...
      const agentBalanceAfter = await ethers.provider.getBalance(
        agent1.address,
      );
    ...
    it("Should emit CommissionWithdrawn event", async function () {
      ...
      await expect(agentProxy.connect(agent1).withdrawCommission())
    ...
    it("Should allow withdrawal when agent is inactive but has commission accrued", async function () {
      ...
      expect(balAfter).to.equal(
        balBefore.add(infoBefore.totalCommissionEarned).sub(gasUsed),
      );
